...
class ProductController extends Controller
{
    public function all()
    {
        ...
        if (request()->has('search_key')) {
            ...
            $check_id = Product::where('id',$key)->exists();
            $check_name = Product::where('product_name',$key)->exists();
            if($check_id){
                $query->where('id', $key);
            }else if($check_name){
                $query->where('product_name', $key);
            }
            else{
    ...
    public function store()
    {
        ...
        if (count($related_images) > 0) {
            ...
            $product->product_url = save_website_url($product->product_url,'products',$product->id);
            $product->save();
